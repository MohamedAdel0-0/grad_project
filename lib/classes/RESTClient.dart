...
class RESTClient {
    ...
    static Future<User> register(User user) async {
        final response = await http.post(Uri.encodeFull(SERVER_URL + 'api/users'),
    ...
    static Future<List<Topic>> getAllTopics() async {
        ...
        if (response.statusCode == 200) 
        {
            ...
            List<Topic> topics = objects.map((dynamic item) => Topic.fromJson(item)).toList();
    ...
    static Future<List<College>> getAllColleges() async {
        ...
        {
            List<dynamic> objects = jsonDecode(response.body)['objects'];
            List<College> colleges = objects.map((dynamic item) => College.fromJson(item)).toList();
    ...
    static Future<Topic> addTopic(Topic topic) async {
        final response = await http.post(Uri.encodeFull(SERVER_URL + 'api/topics'),
    ...
    static Future<Department> addDepartment(Department department) async {
        final response = await http.post(Uri.encodeFull(SERVER_URL + 'api/departments'),
            headers: {'Content-Type': 'application/json'},
    ...
    static Future<College> addCollege(College college) async {
        final response = await http.post(Uri.encodeFull(SERVER_URL + 'api/colleges'),
            headers: {'Content-Type': 'application/json'},
